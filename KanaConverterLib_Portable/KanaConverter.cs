...
namespace KanaConverterLib
{
    public abstract class KanaConverter
    {
        ...
        {
            foreach (var d in Diphthongs) sb.Replace(d.Romaji, toHiragana ? d.Hiragana : d.Katakana);
            foreach (var rk in RomajiKanas) sb.Replace(rk.Romaji, toHiragana ? rk.Hiragana : rk.Katakana);
        }
        ...
        protected string ConvertKanaToRomaji(string text)
        {
            ...
            {
                sb.Replace(d.Hiragana, d.Romaji);
                sb.Replace(d.Katakana, d.Romaji);
            }

            foreach (var rk in RomajiKanas)
            {
                sb.Replace(rk.Hiragana, rk.Romaji);
                sb.Replace(rk.Katakana, rk.Romaji);
            }
        ...
        protected string ConvertHiraganaToKatakana(string text)
        {
            ...
            var katEnum = text.Cast<char>().Select(c => (char)((c > 0x3040 && c < 0x30F7) ? c + 96 : c));
        ...
        protected string ConvertKatakanaToHiragana(string text)
        {
            ...
            var hirEnum = text.Cast<char>().Select(c => (char)((c > 0x30A0 && c < 0x30F4) ? c - 96 : c));
        ...
        {
            return text.Cast<char>().Any(s => s >= 'A' && s <= 'z');
        }

        public static bool ContainsHiraganaCharacters(string text)
        {
            return text.Cast<char>().Any(s => s > 0x3040 && s < 0x3097);
        }

        public static bool ContainsKatakanaCharacters(string text)
        {
            return text.Cast<char>().Any(c => c > 0x30A0 && c < 0x30FA);
        }
        ...
        {
            return LoadRomajiKanas(Data.Diphthongs);
        }

        private IEnumerable<RomajiKana> GetNormalConversion()
        {
            return LoadRomajiKanas(Data.RomajiKanas);
        }

        public static IEnumerable<RomajiKana> LoadRomajiKanas(IEnumerable<string> data)
        {
